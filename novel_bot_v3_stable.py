...
def parse_czbooks_links(soup, limit=20):
    ...
    for a in soup.find_all('a', href=re.compile(r'//czbooks\.net/n/[^/]+$')):
        ...
        if not title or href in seen or is_excluded(title):
            continue
        seen.add(href)
        parent = a.parent.get_text() if a.parent else ''
        done = '\u5df2\u5b8c\u7d50' in parent or '\u5b8c\u7d50' in title
...
def get_hot_list(category=None, sort='', limit=20):
    ...
    return parse_czbooks_links(BeautifulSoup(get_html(url, wait=7), 'html.parser'), limit)
...
def get_weekly_rank(limit=20):
    ...
    for cat in ['xuanhuan', 'xianxia', 'lishi', 'dushi', 'lingyi']:
        if len(results) >= limit:
            break
        try:
            for r in parse_czbooks_links(
                    BeautifulSoup(get_html(f"https://czbooks.net/c/{cat}/weekly", wait=6), 'html.parser'), limit):
...
def search_complete(limit=20):
    ...
    for cat in ['xuanhuan', 'xianxia', 'lishi', 'dushi', 'lingyi', 'wangyou']:
        if len(results) >= limit:
            break
        try:
            soup = BeautifulSoup(get_html(f"https://czbooks.net/c/{cat}/total", wait=6), 'html.parser')
            for a in soup.find_all('a', href=re.compile(r'//czbooks\.net/n/[^/]+$')):
                href = a.get('href', '')
                title = clean_title(a.text)
                if not title or href in seen or is_excluded(title):
                    continue
                parent = a.parent.get_text() if a.parent else ''
                if '\u5df2\u5b8c\u7d50' not in parent and '\u5b8c\u7d50' not in title:
                    continue
                seen.add(href)
...
def parse_book_info(html, url):
    soup = BeautifulSoup(html, 'html.parser')
    title_m = re.search(r'\u300a(.+?)\u300b', soup.title.text if soup.title else '')
    title = title_m.group(1) if title_m else '\u672a\u77e5\u66f8\u540d'
    author_el = soup.find('a', href=re.compile(r'/a/'))
    author = author_el.text.strip() if author_el else '\u672a\u77e5\u4f5c\u8005'
    # intro: prefer meta description
    intro = '\uff08\u7121\u7c21\u4ecb\uff09'
    ...
    if meta and meta.get('content') and len(meta['content']) > 20:
        ...
        for el in soup.find_all(['p', 'div']):
            t = el.get_text().strip()
            if 50 < len(t) < 500 and not el.find('a') and not el.find('ul'):
                if any(s in t for s in ['\u71b1\u9580\u641c\u5c0b', '\u767b\u5165', 'Facebook']):
                    continue
                intro = t[:200]
                break
    # tags: skip hot-search sidebar
    ...
    for a in soup.find_all('a', href=re.compile(r'/hashtag/')):
        ...
        if not tag or len(tag) > 8:
            continue
        if hot_pos > 0 and html.find(tag) >= hot_pos:
            continue
        tags.append(tag)
        if len(tags) >= 5:
            break
...
def make_list_keyboard(results, page=0):
    ...
    for i, r in enumerate(results[start:end]):
        icon = '\u2705' if r['done'] else '\ud83d\udd04'
        buttons.append([{'text': f"{icon} {r['title'][:24]}", 'callback_data': f"book_{start+i}"}])
    nav = []
    if page > 0:
        ...
        if 0 <= idx < len(results):
            ...
            def load_detail():
                try:
                    ...
                    edit_message(chat_id, message_id, format_card(info),
