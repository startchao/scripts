...
def run():
    ...
    def webhook():
        ...
        if 'message' in data:
            threading.Thread(target=handle_message,
                args=(data['message'],), daemon=True).start()
        elif 'callback_query' in data:
            threading.Thread(target=handle_callback,
                args=(data['callback_query'],), daemon=True).start()
        return 'ok'
    ...
    ctx.load_cert_chain('/home/ubuntu/webhook.pem', '/home/ubuntu/webhook.key')
    print("Novel Bot v3 webhook started on port 8443")
